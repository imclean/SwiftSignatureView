...
public protocol SwiftSignatureViewDelegate: class {
    ...
    func swiftSignatureViewFinishDidPanInside(_ view: SwiftSignatureView)
    ...
    */
    ...
    public func setupExistingImage(data:Data) {
        if let imageView = self.imageView {
            ...
            let image = UIImage(data:data)
            self.imageView = UIImageView(frame:CGRect(x: 0, y: 0, width: image!.size.width/UIScreen.main.scale, height: image!.size.height/UIScreen.main.scale))
            imageView?.image = image
            imageView?.contentMode = .scaleAspectFit
            self.addSubview(imageView!)
        }
    ...
    func pan(_ pan:UIPanGestureRecognizer) {
        ...
        case .ended:
            self.delegate?.swiftSignatureViewFinishDidPanInside(self)
            break
